...
class Triangle {
  ...
  static triangleRayIntersection(ray, triangle) {
    ...
    if (t1.area + t2.area + t3.area <= triangle.area + triangle.area / 100) {
      return true;
    }
  }
}
...
class Matrix {
   ...
   * @returns {Matrix}
   */
  static roundMatrix(m, round) {
    let n = new Matrix(
      MatrixVector4D.roundMatrixVector4D(m.i, round),
      MatrixVector4D.roundMatrixVector4D(m.j, round),
      MatrixVector4D.roundMatrixVector4D(m.k, round)
    );
    return n;
  }
}
  ...
  constructor(position, width, height, ctx, pixelSize) {
    ...
    for (let i = 0; i <= this.width; i = i + this.pixelSize) {
      for (let j = 0; j <= this.height; j = j + this.pixelSize) {
        this.pixels.push(
          new Vector3D(i - this.halfWidth, this.halfHeight - j, position.z)
        );
      }
    }
    this.originalPixels = [];
...
let zRotazion = 90 * (Math.PI / 180);
let xRotation = 0;
let yRotation = 0;

let rzm = new Matrix(
  new MatrixVector4D(Math.cos(zRotazion), Math.sin(zRotazion), 0, 0),
  new MatrixVector4D(-1 * Math.sin(zRotazion), Math.cos(zRotazion), 0, 0),
  new MatrixVector4D(0, 0, 1, 0)
);
console.log(Math.sin(zRotazion) * -1);
rzm = Matrix.roundMatrix(rzm, 0.001);
console.log(rzm);

let r = 10 * (Math.PI / 180);
let cameraMatrix = new Matrix(
  new MatrixVector4D(Math.cos(r), Math.sin(r), 0, 0),
  new MatrixVector4D(-Math.sin(r), Math.cos(r), 0, 0),
  new MatrixVector4D(0, 0, 1, 0)
);
let film = new Film(new Vector3D(0, 0, 50), 600, 400, ctx, 3);
